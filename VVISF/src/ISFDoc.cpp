...
ISFDoc::ISFDoc(const string& inFSContents,
...
bool ISFDoc::generateShaderSource(string* outFragSrc,
                                  string* outVertSrc,
                                  GLVersion& inGLVers,
  ...
  newFragShaderSrc.reserve(uint32_t(1.5 * (fsVarDeclarations.size() + _fragShaderSource->size())));
  {
    ...
    //	now i have to find-and-replace the shader source for various things- make a copy of the raw source and work from
    // that.
      ...
      if (tmpRange.loc != string::npos) {
        ...
        if (varArrayCount != 2 && varArrayCount != 3) {
          if (_throwExcept) {
            ...
            string tmpErrLog = FmtString("ERROR: 0:%d: IMG_PIXEL has wrong number of arguments",
                                         NumLines(string(modSrcString, 0, tmpRange.max())) + 1);
          ...
          if (imgBuffer == nullptr || imgBuffer->desc.target == GLBuffer::Target_2D) {
            if (varArrayCount == 3) {
              newFuncString =
                  FmtString("VVSAMPLER_2DBYPIXEL(%s, _%s_imgRect, _%s_imgSize, _%s_flip, %s, %s)", samplerNameC,
                            samplerNameC, samplerNameC, samplerNameC, samplerCoordC, varArray[2].c_str());
              requires2DBiasMacro = true;
            } else {
              newFuncString = FmtString("VVSAMPLER_2DBYPIXEL(%s, _%s_imgRect, _%s_imgSize, _%s_flip, %s)", samplerNameC,
                                        samplerNameC, samplerNameC, samplerNameC, samplerCoordC);
              requires2DMacro = true;
            }
          } else {
            if (varArrayCount == 3) {
              newFuncString =
                  FmtString("VVSAMPLER_2DRECTBYPIXEL(%s, _%s_imgRect, _%s_imgSize, _%s_flip, %s, %s)", samplerNameC,
                            samplerNameC, samplerNameC, samplerNameC, samplerCoordC, varArray[2].c_str());
              requires2DRectBiasMacro = true;
            } else {
              newFuncString = FmtString("VVSAMPLER_2DRECTBYPIXEL(%s, _%s_imgRect, _%s_imgSize, _%s_flip, %s)",
                                        samplerNameC, samplerNameC, samplerNameC, samplerNameC, samplerCoordC);
              requires2DRectMacro = true;
            }
          }

          modSrcString.replace(fullFuncRangeToReplace.loc, fullFuncRangeToReplace.len, newFuncString, 0,
                               newFuncString.size());
        }
      }
    } while (tmpRange.loc != string::npos);
    ...
    do {
      ...
      if (tmpRange.loc != string::npos) {
        ...
        if (varArrayCount != 2 && varArrayCount != 3) {
          if (_throwExcept) {
            map<string, string> tmpErrDict;
            string tmpErrLog = FmtString("ERROR: 0:%d: IMG_NORM_PIXEL has wrong number of arguments",
                                         NumLines(string(modSrcString, 0, tmpRange.max())) + 1);
            tmpErrDict.insert(pair<string, string>(string("fragErrLog"), tmpErrLog));
            tmpErrDict.insert(pair<string, string>(string("fragSrc"), modSrcString));
            if (_path != nullptr)
          ...
          if (imgBuffer == nullptr || imgBuffer->desc.target == GLBuffer::Target_2D) {
            if (varArrayCount == 3) {
              newFuncString =
                  FmtString("VVSAMPLER_2DBYNORM(%s, _%s_imgRect, _%s_imgSize, _%s_flip, %s, %s)", samplerNameC,
                            samplerNameC, samplerNameC, samplerNameC, samplerCoordC, varArray[2].c_str());
              requires2DBiasMacro = true;
            } else {
              newFuncString = FmtString("VVSAMPLER_2DBYNORM(%s, _%s_imgRect, _%s_imgSize, _%s_flip, %s)", samplerNameC,
                                        samplerNameC, samplerNameC, samplerNameC, samplerCoordC);
              requires2DMacro = true;
            }
          } else {
            if (varArrayCount == 3) {
              newFuncString =
                  FmtString("VVSAMPLER_2DRECTBYNORM(%s, _%s_imgRect, _%s_imgSize, _%s_flip, %s, %s)", samplerNameC,
                            samplerNameC, samplerNameC, samplerNameC, samplerCoordC, varArray[2].c_str());
              requires2DRectBiasMacro = true;
            } else {
              newFuncString = FmtString("VVSAMPLER_2DRECTBYNORM(%s, _%s_imgRect, _%s_imgSize, _%s_flip, %s)",
                                        samplerNameC, samplerNameC, samplerNameC, samplerNameC, samplerCoordC);
              requires2DRectMacro = true;
            }
          }
    ...
    do {
      ...
      if (tmpRange.loc != string::npos) {
        ...
        if (varArrayCount != 1) {
          if (_throwExcept) {
            map<string, string> tmpErrDict;
            string tmpErrLog = FmtString("ERROR: 0:%d: IMG_THIS_PIXEL has wrong number of arguments",
                                         NumLines(string(modSrcString, 0, tmpRange.max())) + 1);
            tmpErrDict.insert(pair<string, string>(string("fragErrLog"), tmpErrLog));
            tmpErrDict.insert(pair<string, string>(string("fragSrc"), modSrcString));
            if (_path != nullptr) {
          ...
          if (find(imgThisPixelSamplerNames.begin(), imgThisPixelSamplerNames.end(), samplerName) ==
              imgThisPixelSamplerNames.end())
    ...
    do {
      ...
      if (tmpRange.loc != string::npos) {
        ...
        if (varArrayCount != 1) {
          if (_throwExcept) {
            map<string, string> tmpErrDict;
            string tmpErrLog = FmtString("ERROR: 0:%d: IMG_THIS_NORM_PIXEL has wrong number of arguments",
                                         NumLines(string(modSrcString, 0, tmpRange.max())) + 1);
            tmpErrDict.insert(pair<string, string>(string("fragErrLog"), tmpErrLog));
            tmpErrDict.insert(pair<string, string>(string("fragSrc"), modSrcString));
            if (_path != nullptr)
          ...
          if (find(imgThisNormPixelSamplerNames.begin(), imgThisNormPixelSamplerNames.end(), samplerName) ==
              imgThisNormPixelSamplerNames.end())
    ...
    do {
      ...
      if (tmpRange.loc != string::npos) {
        ...
        if (varArrayCount != 1) {
          if (_throwExcept) {
            map<string, string> tmpErrDict;
            string tmpErrLog = FmtString("ERROR: 0:%d: IMG_SIZE has wrong number of arguments",
                                         NumLines(string(modSrcString, 0, tmpRange.max())) + 1);
            tmpErrDict.insert(pair<string, string>(string("fragErrLog"), tmpErrLog));
            tmpErrDict.insert(pair<string, string>(string("fragSrc"), modSrcString));
            if (_path != nullptr)
  ...
  newVertShaderSrc.reserve(uint32_t(2.5 * (vsVarDeclarations.size() + _vertShaderSource->size())));
  {
    ...
    //	add the variables for values corresponding to buffers from IMG_THIS_PIXEL and IMG_THIS_NORM_PIXEL in the frag
    // shader
    ...
    for (const auto& it : imgThisPixelSamplerNames) {
      ...
      // vec2(((isf_fragCoord.x/(_%s_imgSize.x)*_%s_imgRect.z)+_%s_imgRect.x),
      // (_%s_imgRect.w-(isf_fragCoord.y/(_%s_imgSize.y)*_%s_imgRect.w)+_%s_imgRect.y)) :
      // vec2(((isf_fragCoord.x/(_%s_imgSize.x)*_%s_imgRect.z)+_%s_imgRect.x),
      // (isf_fragCoord.y/(_%s_imgSize.y)*_%s_imgRect.w)+_%s_imgRect.y);\n",samplerName,samplerName,samplerName,samplerName,samplerName,samplerName,samplerName,samplerName,samplerName,samplerName,samplerName,samplerName,samplerName,samplerName,samplerName));
      //	the floor(XXX + 0.5) trick used here is meant to be equivalent to round(XXX), which doesn't exist in
      // GLSL
      newVertShaderSrc.append(FmtString(
          "\t_%s_texCoord = (_%s_flip) \
          ...
          samplerName, samplerName, samplerName, samplerName, samplerName, samplerName, samplerName, samplerName,
          samplerName, samplerName, samplerName, samplerName, samplerName, samplerName, samplerName));
    }
    ...
    for (const auto& it : imgThisNormPixelSamplerNames) {
      ...
      if (imgBuffer == nullptr || imgBuffer->desc.target == GLBuffer::Target_2D) {
        newVertShaderSrc.append(
            FmtString("\t_%s_normTexCoord = (_%s_flip) \
					...
					: vec2((((isf_FragNormCoord.x*_%s_imgSize.x)/_%s_imgSize.x*_%s_imgRect.z)+_%s_imgRect.x), ((isf_FragNormCoord.y*_%s_imgSize.y)/_%s_imgSize.y*_%s_imgRect.w)+_%s_imgRect.y);\n",
                      samplerName, samplerName, samplerName, samplerName, samplerName, samplerName, samplerName,
                      samplerName, samplerName, samplerName, samplerName, samplerName, samplerName, samplerName,
                      samplerName, samplerName, samplerName, samplerName, samplerName));
      } else {
        newVertShaderSrc.append(
            FmtString("\t_%s_normTexCoord = (_%s_flip) \
...
void ISFDoc::_initWithRawFragShaderString(const string& inRawFile) {
  //	isolate the JSON blob that should be at the beginning of the file in a comment, save it as one string- save
  // everything else as the raw shader source string
  auto openCommentIndex = inRawFile.find("/*");
  auto closeCommentIndex = inRawFile.find("*/");
  if (openCommentIndex == string::npos || closeCommentIndex == string::npos || closeCommentIndex < openCommentIndex) {
    if (_throwExcept) {
      if (_path != nullptr)
  ...
  //	we need to advance 'closeCommentLineEnd' to include both the full "close comment" as well as the next "line
  // break"
  ...
  */
  ...
  if (anObj != nullptr) {
        ...
        if (!cubeFlagJ.is_null()) {
          ...
          if (partialPathsJ.is_null() || !partialPathsJ.is_array()) {
            ...
            if (importedBuffer == nullptr) {
              ...
              // ISFAttr		newAttrib(samplerNameJ.get<string>(), string(""), string(""), ISFValType_Image,
              // ISFNullVal(), ISFNullVal(), ISFImageVal(importedBuffer), ISFNullVal(), nullptr, nullptr); ISFAttrRef
              // newAttribRef = make_shared<ISFAttr>(newAttrib);
              ISFAttrRef newAttribRef = make_shared<ISFAttr>(
                  samplerNameJ.get<string>(), string(""), string(""), ISFValType_Cube, ISFNullVal(), ISFNullVal(),
                  ISFImageVal(importedBuffer), ISFNullVal(), nullptr, nullptr);
        ...
        //	else it's just a normal image
          ...
          if (partialPathJ.is_null() || !partialPathJ.is_string()) {
            ...
            if (importedBuffer == nullptr) {
              ...
              // ISFAttr		newAttrib(samplerNameJ.get<string>(), fullPath, string(""), ISFValType_Image,
              // ISFNullVal(), ISFNullVal(), ISFImageVal(importedBuffer), ISFNullVal(), nullptr, nullptr); ISFAttrRef
              // newAttribRef = make_shared<ISFAttr>(newAttrib);
              ISFAttrRef newAttribRef =
                  make_shared<ISFAttr>(samplerNameJ.get<string>(), fullPath, string(""), ISFValType_Image, ISFNullVal(),
                                       ISFNullVal(), ISFImageVal(importedBuffer), ISFNullVal(), nullptr, nullptr);
              _imageImports.emplace_back(newAttribRef);
            }
          }
        }
    ...
    //	if i'm importing files from a dictionary, execute the block on all the elements in the dict (each element is
    // another dict describing the thing to import)
      ...
      for (auto passIt = anObj.begin(); passIt != anObj.end(); ++passIt) {
        ...
        if (passTarget.is_string()) {
          ...
          if (targetBuffer == nullptr) {
            ...
            //	if there's a valid PERSISTENT flag and it's indicating positive, add the new target buffer as a
            // persistent buffer
  ...
  if (inputsArray != nullptr && inputsArray.type() == json::value_t::array) {
    ...
    bool clampMin = false, clampMax = false;
    ...
    for (auto it = inputsArray.begin(); it != inputsArray.end(); ++it) {
      ...
      if (typeStringJ == "image") {
        ...
        if ((minVal.isNullVal() && !maxVal.isNullVal()) || (!minVal.isNullVal() && maxVal.isNullVal())) {
          minVal = ISFNullVal();
          maxVal = ISFNullVal();
        }
      ...
      json tmpObj;
      tmpObj = inputDict.value("CLAMP_MIN", json());
      if (tmpObj.is_boolean())
        clampMin = tmpObj.get<bool>();
      tmpObj = inputDict.value("CLAMP_MAX", json());
      if (tmpObj.is_boolean())
        clampMax = tmpObj.get<bool>();

      //	if i'm here, i've got all the data necessary to create an input/attribute and need to do so
      ISFAttrRef newAttribRef = make_shared<ISFAttr>(inputKeyJ, descString, labelString, newAttribType, minVal, maxVal,
                                                     defVal, idenVal, &labelArray, &valArray);
      newAttribRef->setUnit(unit);
      newAttribRef->setClampMin(clampMin);
      newAttribRef->setClampMax(clampMax);
...
bool ISFDoc::_assembleShaderSource_VarDeclarations(string* outVSString,
                                                   string* outFSString,
                                                   GLVersion& inGLVers,
                                                   const bool& inVarsAsUBO) {
  lock_guard<recursive_mutex> lock(_propLock);
  ...
  //	we're going to work this by assembling an array of strings, one for each line- have the vector reserve space for
  // enough strings
  vector<string> vsDeclarations;
  vector<string> fsDeclarations;
  vector<string> uboDeclarations;

  vsDeclarations.reserve(_inputs.size() + _imageImports.size() + _persistentPassTargets.size() +
                         _tempPassTargets.size() + 9);
  fsDeclarations.reserve(vsDeclarations.capacity());
  uboDeclarations.reserve(vsDeclarations.capacity());

  //	frag shader always needs an output, which we're naming gl_FragColor so shaders written against GL 2.1 will work.
  // we'll use a #define in the shader source to make the shader precompiler change gl_FragColor to isf_FragColor
  switch (inGLVers) {
    case GLVersion_Unknown:
    case GLVersion_2:
    case GLVersion_ES:
...
ostream& operator<<(ostream& os, const ISFDoc& n) {
  ...
  if (tmpInputs.size() > 0) {
    ...
    for (const auto& it : tmpInputs) {
      ...
      if (currentVal.type() != ISFValType_None || minVal.type() != ISFValType_None ||
          maxVal.type() != ISFValType_None || defaultVal.type() != ISFValType_None ||
          identityVal.type() != ISFValType_None || labels.size() > 0 || vals.size() > 0) {
        os << "\t\t\t";
        needsANewline = true;
      }
